...
{	
	...
	/*This function created by Mizanur Rahman on 2014-05-30 12:29 am
	for taking all campaigns from GetResponse account.
	*/
	public function get_campaign_dropdown()
	{
		$campaign_dropdown = NULL;
		$request  = $this->prepRequest('get_campaigns');
		$response = $this->execute($request);
		$campaigns = (array)$response;
		if(is_array($campaigns))
		{
			foreach($campaigns as $key=>$value)
			{
				$campaign_dropdown[$value->name] = $value->name;
			}
		}
